...
from sbi_for_diffusion_models.run_config import T_MAX
...
class TestSimulateTrainingSessionsShapes:
    ...
    def test_mixed_p_success_keeps_standard_shape(self):
        theta, x = simulate_training_sessions(
            self.prior,
            num_sessions=self.N,
            num_trials=self.T,
            ...
            device=self.device,
            mu_sensory=1.0,
            p_success=(1.0, 0.8, 0.7, 0.6),
            P=self.P,
            log_rt=True,
            seed=0,
        )
        assert theta.shape == (self.N, 5)
        assert x.shape == (self.N, self.T * TRIAL_DIM)
...
class TestTimeoutHandling:
    ...
    def test_forced_trials_have_tmax_rt(self, device):
        """With huge B and tiny v, most trials time out and should be forced to T_MAX."""
